...
int main(int argc, char* argv[]) {
  ...
  if (ntot == 0)
    ntot = 1; /* avoid divide by 0; print 0 for all conuts */
...
int myftw(char* pathname, Myfunc* func) {
  ...
  if (pathlen <= strlen(pathname)) {
    ...
    if ((fullpath = (char*)realloc(fullpath, pathlen)) == nullptr)
 ...
 */
int dopath(Myfunc* func) { /* we return whatever func() returns */
  struct stat statbuf;
  struct dirent* dirp;
  DIR* dp;
  int ret, n;
  ...
  if ((dp = opendir(fullpath)) == nullptr) /* can't read directory */
